...
void main(){
  ...
  // Light v2.1.9: target the tonal zones used by Adobe's Basic-panel model.
  ...
  // Color Mixer v2.1.9: use a lightly spatial-smoothed selector, normalize only
...
function wireUI(){
  ...
  if(windowBtn) windowBtn.addEventListener('click',e=>{
    ...
    popupUrl.searchParams.set('v','2.1.9');
...
try{initGL();loadCustomPresets();populatePresetSelect();buildControls();wireUI();const windowBtn=$('#btnWindow');if(isPopup&&windowBtn){windowBtn.innerHTML='<span class="window-icon">✕</span><span class="window-copy"><strong>Đóng cửa sổ lớn</strong><small>Quay lại panel Photopea</small></span>';windowBtn.title='Đóng cửa sổ Camera Raw Studio';}drawCurve();updateUndoButtons();setStatus(`Sẵn sàng • WebGL2 • v2.1.9 • giới hạn ${maxTextureSize}px`);}catch(e){setStatus(e.message,true);console.error(e);alert(e.message)}
