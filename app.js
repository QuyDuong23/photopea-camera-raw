...
const defs = {
  ...
  whites:{label:'Whites',min:-100,max:100,step:1,group:'light',help:'Điều chỉnh điểm trắng và vùng sáng nhất của ảnh'}, blacks:{label:'Blacks',min:-100,max:100,step:1,group:'light',help:'Điều chỉnh điểm đen và vùng tối nhất của ảnh'},
...
const sliderVisuals = {
  whites:{gradient:'linear-gradient(90deg,#4b4b4b 0%, #8a8a8a 42%, #d7d7d7 75%, #ffffff 100%)', accent:'#f2f2f2'},
  blacks:{gradient:'linear-gradient(90deg,#050505 0%, #242424 35%, #555555 72%, #8a8a8a 100%)', accent:'#8a8a8a'},
...
float adjustWhites(float y,float amount){
  float mask=smoothstep(0.48,0.96,y);
  return amount>=0.0 ? y+(1.0-y)*amount*mask*0.92 : y+y*amount*mask*0.52;
}
float adjustBlacks(float y,float amount){
  float mask=1.0-smoothstep(0.035,0.48,y);
  return amount>=0.0 ? y+(1.0-y)*amount*mask*0.30 : y+y*amount*mask*0.92;
}
...
void main(){
  ...
  // Whites / Blacks v2.1.5: independent white-point and black-point controls.
  // The masks stay concentrated in the brightest and darkest tonal ranges,
  // while luminance is remapped without rotating hue like Photoshop Camera Raw.
  y=lum(c); c=setLuminancePreserveHue(c,adjustBlacks(y,uBlacks));
  y=lum(c); c=setLuminancePreserveHue(c,adjustWhites(y,uWhites));
  c=(c-.5)*(1.0+uDehaze*.45)+.5;

  // Color Mixer v2.1.5: use a lightly spatial-smoothed selector, normalize only
...
function wireUI(){
  ...
  if(windowBtn) windowBtn.addEventListener('click',e=>{
    ...
    popupUrl.searchParams.set('v','2.1.5');
...
try{initGL();loadCustomPresets();populatePresetSelect();buildControls();wireUI();const windowBtn=$('#btnWindow');if(isPopup&&windowBtn){windowBtn.innerHTML='<span class="window-icon">✕</span><span class="window-copy"><strong>Đóng cửa sổ lớn</strong><small>Quay lại panel Photopea</small></span>';windowBtn.title='Đóng cửa sổ Camera Raw Studio';}drawCurve();updateUndoButtons();setStatus(`Sẵn sàng • WebGL2 • v2.1.5 • giới hạn ${maxTextureSize}px`);}catch(e){setStatus(e.message,true);console.error(e);alert(e.message)}
